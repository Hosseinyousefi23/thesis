...
نامگذاری فرایند تصمیم‌گیری مارکوف اشاره به این موضوع دارد که این سیستم\nf ها دارای \مهم{ویژگی مارکوف \پاورقی{Marcov Property}}  هستند، بدین معنا که تابع انتقال آن\nf ها تنها به حالت فعلی سیستم و آخرین عمل انجام شده وابسته است، و نسبت به حالت\nf ها و اعمال قبلی مستقل است.
\section{عناصر اصلی یادگیری تقویتی}
\subsection{خط‌مشی}
خط‌مشی\پاورقی{Policy} نحوه رفتار عامل یادگیری را در یک زمان خاص، مشخص می‌کند و هسته اصلی رفتار یک عامل یادگیری تقویتی است. خط‌مشی به تنهایی برای تعیین رفتار کافی است. به عنوان یک تعریف غیر دقیق، خط‌مشی، نگاشتی از حالت‌های مدل شده از محیط به عملی است که باید در آن حالت انجام شود.
...
\پاورقی{Future Discounted Return}
 یا به اختصار، عایدی \پاورقی{Return}، در زمان $t$ به شکل
...
\textit{خط\nf مشی بهینه} 
\پاورقی{Optimal Policy}
گفته می شود. برای MDP های متناهی، می\nf توانیم خط\nf مشی بهینه را به صورت زیر تعریف کنیم
\شروع{تعریف}
...
در این حالت خاص، معادله بلمن را می توان به فرم ویژه ای نوشت که با نام
 ...
 \پاورقی{Bellman Optimality Equation}
شناخته می شود.
...
\پاورقی{Iterative policy evaluation} یا به طور خلاصه 
ارزیابی خط مشی \پاورقی{Policy Evaluation}
...
$v_{\pi}$
 را برای یک خط‌مشی معین \پاورقی{Deterministic} دلخواه 
 $\pi$
...
در الگوریتم تکرار خط مشی  \پاورقی{Policy iteration}هنگامی‌که یک خط‌مشی
$\pi$
 با استفاده از 
$v_{\pi}$
...
در واقع، عملیات ارزیابی خط مشی را می توان پس از یک بار رفت و برگشت روی فضای حالت (یک به روز رسانی در هر حالت)، موقف کرد بدون از دست دادن تضمین همگرایی به خط مشی بهینه 
این الگوریتم،
\textit{تکرار ارزش}
\پاورقی{Value Iteration}
 نامیده می شود. آی تی
را می توان به عنوان یک عملیات به روزرسانی ساده نوشت که ترکیبی از بهبود خط مشی و
...
%
یک اشکال عمده در روشهای DP که ما تاکنون بحث کردیم این است که آنها شامل عملیات هایی روی تمام مجموعه حالت ها هستند
اگر فضای حالت بسیار بزرگ باشد
حتی یک بار رفت و برگشت می تواند بسیار پرهزینه باشد


الگوریتم های ناهمزمان DP الگوریتم های تکرار شونده درجا هستند که مقید به رفت و برگشت سیستماتیک روی تمام مجموعه حالت ها نیستند. این الگوریتم ها ارزش حالت ها را به بدون هیچ ترتیب منظمی و با استفاده از هر تخمینی از ارزش حالت های دیگر که در دسترس باشد، به روز می کنند
به هر حال ارزش برخی حالت ها ممکن است قبل از اینکه ارزش دیگران یکبار به روز شود، چندین بار به روز شود. با این حال ، برای همگرایی صحیح، یک الگوریتم ناهمزمان باید به روز رسانی مقادیر همه حالت ها را ادامه دهد و نمی تواند حالتی را نادیده بگیرد.
