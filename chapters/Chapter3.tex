...
{الگوریتم دسته ای}
\ورودی{یک انتقال 
	$x_t, a_t, r_t, x_{t+1}, \gamma_t \in [0,1]$
}
\دستور{
	$Q(x_{t+1}, a) \triangleq \sum_{i} z_i p_i (x_{t+1}, a)$
}
\دستور{
$a^* \longleftarrow arg \max_{a} Q(x_{t+1}, a)$
}
\دستور{
...
\دستور{
	$\hat{\mathcal{T}}_{z_j} \longleftarrow \left[r_t + \gamma_t z_j \right]^{V_{MAX}}_{V_{MIN}}$
}
\دستور{
$b_j \longleftarrow \dfrac{(\hat{\mathcal{T}}_{z_j} - V_{MIN})}{\Delta z}$
}
\دستور{
	$l \longleftarrow \floor{b_j}, u \longleftarrow \ceil{b_j}$
}

\دستور{
	$m_l \longleftarrow m_l + p_j(x_{t+1}, a^*)(u-b_j)$
}
\دستور{
	$m_u \longleftarrow m_u + p_j(x_{t+1},a^*)(b_j - l)$
}
\پایان‌به‌ازای
\خروجی{
	$\sum_{i} m_i log p_i (x_t, a_t)$
}
\پایان{الگوریتم}
